...
export const MostPopular = () => {
  ...
  useEffect(() => {
    const fetchMostPopular = async (limit, page) => {
      try {
        setLoading(true);
        const data = await fetchWithErrorHandling(
          `${API_ENDPOINTS.mostPopular}?limit=${limit}&page=${page}`
        );
        ...
        console.error("Failed to fetch most popular manga:", err);
        setError("Failed to load manga. Please try again later.");
      } finally {
        setLoading(false);
      }
    };
    fetchMostPopular(40, currentPage);
  }, [currentPage]);
